...
layui.define('jquery', function(exports){
    ...
    Element.prototype.tabAdd = function(filter, options){
        ...
        var lis = titElem.children('li')
        if(lis.length > 21 ){ //固定最大显示页签数量22
            layer.msg(bt.lang('common.element.tabTitleFullMsg'), {//'任务栏已满，请删除一些'
                time: 1000 //1s后自动关闭
            });
            return
    ...
    var NAV_ELEM = '.layui-nav', NAV_ITEM = 'layui-nav-item', NAV_BAR = 'layui-nav-bar'
        ...
        ,call = {
            ...
            tabClick: function(e, index, liElem,isClosed){
                ...
                othis.parent().removeClass('t2-tab-open');
                //解决火狐刷新表格的问题 开始
                ...
                if(span.length !== 0 && !isClosed){ //判断是否有两行选项卡菜单
                    ...
                    if( spanTitle ){
                        ...
                        if( othisOffestTop - parentsTop > height -1 ){ //如果是在第二行，就执行下面代码，否则不用查找
                            ...
                            var firstli = span.siblings('[lay-id="0"]'); //更改不翻页功能，直接插到第一个位置
                            firstli.after(othis);//插入在第一个选项卡之后
                        }
                    }
                }

                if( layId == '0' && othis.parent().children('li').length == 1){
            ...
            ,tabDelete: function(e, othis){
                ...
                if( li.attr('upload') == 'true' ){ //解决上传了附件未提交时的提示
                    layer.confirm(bt.lang('common.element.uploadFileNotSaveMsg')+'？', {//'上传的附件未保存，是否离开当前页面
                        btn: [bt.lang('abBillPledgeApply.interiorApply.yes'),bt.lang('abBillPledgeApply.interiorApply.no')]//['是','否'] //按钮
                    }, function(){
            ...
            ,tabAuto: function(){
                ...
                $('.layui-tab').each(function(){
                    var othis = $(this)
                        ...
                        ,span = $('<span class="layui-unselect layui-tab-bar font_family icon-yeqianlan_zhankai" '+ STOPE +'>' +
                        // '<i '+ STOPE +' class="layui-icon">&#xe61a;</i>' +
                        '</span>');
                    ...
                    if(title.prop('scrollWidth') > title.outerWidth()+1 || title.hasClass("page")){
                        ...
                        span.on('click', function(e){
                            ...
                            if (!this.title) {
                                $(this).removeClass('icon-yeqianlan_zhankai').addClass('icon-yeqianlan_shouqi');
                                // $(this).prevAll('li').addClass('t2-tab-open'); //hover 样式不一样的
                                title.addClass('t2-tab-open')
                            }else {
                                $(this).addClass('icon-yeqianlan_zhankai').removeClass('icon-yeqianlan_shouqi');
                                // $(this).siblings('li').removeClass('t2-tab-open')
                                title.removeClass('t2-tab-open')
                            }
                            // $(this)[ ? 'removeClass' : 'addClass']('icon-yeqianlan_zhankai')
                            this.title = this.title ? '' :  bt.lang('common.element.shrink');//'收缩'
                        });
                    } else {
                ...
                var TIME = 200, timer, timerMore, timeEnd, follow = function(bar, nav){
                    ...
                    if(nav.hasClass(NAV_TREE)){
                        ...
                        timerMore = setTimeout(function(){
                            ...
                            if (othis.parents('.t2-header')[0]){
                                othis.addClass('nav-item-hover');
                            }
                        }, 300);
                    }
                }
                        ...
                        itemElem.on('mouseenter', function(){
                            ...
                            if(!othis.hasClass(NAV_TREE)){
                                ...
                                timerMore = setTimeout(function(){
                                    ...
                                    othis.find('.nav-item-hover').removeClass('nav-item-hover');
                                }, 300);
                            }
                        });
    ...
    var tabDel = function (li,index,currentItem) {
        ...
        li.remove();
        currentItem.remove();
        if (parent.children("li").length == 1){ //当页面上只有一个时，删除选中样式
            parent.children("li").removeClass(THIS)
        }
        setTimeout(function(){
            call.tabAuto();
        }, 50);
