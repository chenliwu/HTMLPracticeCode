...
layui.define(['layer', 'jquery'], function (exports) {
    ...
    var $ = layui.jquery
        ...
        //字符常量
        , MOD_NAME = 'uploadExcel', ELEM = '.layui-upload', THIS = 'layui-this', SHOW = 'layui-show',
    ...
    Class.prototype.upload = function (files, type) {
        var that = this
            ...
            , ajaxSend = function () {
                ...
                //上传前的追加参数
                var appendCustomParame = options.appendCustomParame && options.appendCustomParame(args);
                ...
                layui.each(items, function (index, file) {
                    ...
                    //追加自定义的参数
                    if (appendCustomParame) {
                        layui.each(appendCustomParame, function (key, value) {
                            value = typeof value === 'function' ? value() : value;
                    ...
                    $.ajax({
                        ...
                        , error: function () {
                            aborted++;
                            that.msg(bt.lang("common.upload.request.interface.exception"));
                            error(index);
                            allDone();
                        }
                    });
            ...
            , iframeSend = function () {
                ...
                Class.timer = setInterval(function () {
                    ...
                    try {
                        ...
                        that.msg(bt.lang("common.upload.get.response.info.exception"));
                        clearInterval(Class.timer);
                        error();
                    }
                    if (res) {
            ...
            , done = function (index, res) {
                ...
                if (typeof res !== 'object') {
                    var regexZero = /:+(\-?\d+\.+[0-9]*[0])+\,/g;
                    try {
                        res = res.replace(regexZero, ":\"$1\",");//将小数位最后为0的转字符串
                        res = JSON.parse(res);
                    } catch (e) {
                        res = {};
                        return that.msg(bt.lang("common.upload.return.valid.json"));
                    }
                }
        ...
        switch (options.accept) {
            case 'file': //一般文件
                if (exts && !RegExp('\\w\\.(' + exts + ')$', 'i').test(escape(value))) {
                    that.msg(bt.lang("common.upload.file.contains.unsupported.format"));
                    return elemFile.value = '';
                }
                break;
            case 'video': //视频文件
                if (!RegExp('\\w\\.(' + (exts || 'avi|mp4|wma|rmvb|rm|flash|3gp|flv') + ')$', 'i').test(escape(value))) {
                    that.msg(bt.lang("common.upload.file.video.contains.unsupported.format"));
                    return elemFile.value = '';
                }
                break;
            case 'audio': //音频文件
                if (!RegExp('\\w\\.(' + (exts || 'mp3|wav|mid') + ')$', 'i').test(escape(value))) {
                    that.msg(bt.lang("common.upload.file.audio.contains.unsupported.format"));
                    return elemFile.value = '';
                }
                break;
            default: //图片文件
                ...
                if (check) {
                    that.msg(bt.lang("common.upload.file.image.contains.unsupported.format"));
                    return elemFile.value = '';
                }
                break;
        }
        ...
        if (options.number && that.fileLength > options.number) {
            return that.msg(bt.lang("common.upload.file.number.cannot.exceed") + options.number);
        }
        ...
        if (options.size > 0 && !(device.ie && device.ie < 10)) {
            ...
            if (limitSize) return that.msg(bt.lang("common.upload.file.size.cannot.exceed") + limitSize);
        }
        send();
    };
