...
;!function(){
  ...
  */
  ...
  Class.prototype.config = {
    ...
    ,lang:window.top.webLocale?(window.top.webLocale == 'en_US'?'en':'cn'):'cn'// 'cn' //语言，只支持cn/en，即中文和英文
    ,theme: 'default' //主题
  ...
  Class.prototype.list = function(type, index){
    ...
    if(type === 'year'){ //年列表
      ...
      lay.each(new Array(12), function(i){
        ...
        if(listYM[0] < that.firstDate.year){
          ...
          if (ymd.month == that.firstDate.month){ //新增月份相同的判断，解决最小月份不能点击的问题 liuyj 2020-10-13
              ymd.date = that.firstDate.date
          }else {
            ymd.date = options.max.date;
          }
        }
    ...
    if(type === 'year' || type === 'month'){
      ...
      lay(ul).find('li').on('click', function(){
        ...
        // 点击月份日期，直接执行关闭功能 liuyj 2019-04-17
        if (options.type === 'month' && type !== 'year'){ //
            that.tool(lay(that.footer).find('.laydate-btns-confirm')[0], 'confirm');
        }
      });
    } else {
  ...
  Class.prototype.events = function(){
    ...
    ,showEvent = function(elem, bind){
      elem.on(options.trigger, function(){
        ...
        //如果只读的，不生成弹框
        var readyonly = lay(this).attr('readonly');
